# Experimental Design Models: ANOVA and ANCOVA {#sec-design-models}
...
## Design, randomization, and the experimental unit
...
::: {.callout-important title="Avoid pseudoreplication"}
...
::: {#exm-blocked-experiment}

## A Randomized Complete Block Design

Suppose five production batches each provide three comparable units. Within every batch, randomly assign one unit to each of three treatments. There are 15 experimental units and five replications per treatment. An additive fixed-block model is

$$
Y_{bj}=\mu+\gamma_b+\tau_j+\varepsilon_{bj},\qquad
\varepsilon_{bj}\overset{\mathrm{iid}}{\sim}N(0,\sigma^2).
$$
...
```{r}
set.seed(8561)
blocked <- data.frame(block = factor(rep(1:5, each = 3)),
                      unit = rep(1:3, 5))
# Randomize treatment labels among the three units in each block.
blocked$treatment <- factor(
  unlist(lapply(1:5, function(b) sample(c("A", "B", "C")))),
  levels = c("A", "B", "C"))
block_effect <- c(-3, -1, 0, 2, 4)
treatment_effect <- c(A = 0, B = 1.5, C = 3)
blocked$y <- 10 + block_effect[as.integer(blocked$block)] +
  treatment_effect[blocked$treatment] + rnorm(nrow(blocked), sd = 0.8)
fit_block_only <- lm(y ~ block, data = blocked)
fit_block_treatment <- lm(y ~ block + treatment, data = blocked)
anova(fit_block_only, fit_block_treatment)
```

```{r}
#| label: fig-blocked-comparison
#| fig-cap: "Simulated responses within five blocks. Comparing treatments within the same block avoids attributing the large between-batch shifts to treatment. The connecting lines guide comparisons; the model does not assert a linear treatment scale."
#| fig-width: 6
#| fig-height: 4
with(blocked, interaction.plot(treatment, block, y, type = "b", pch = 16,
                               col = hcl.colors(5, "Dark 3"), lwd = 1.5,
...
subject to the model constraints. This is a general linear hypothesis and can be tested by an $F$ test.

**One-Way ANOVA as Regression With Indicators**

Suppose there are three groups: A, B, and C. Using treatment coding with group A as the reference group, we may write
...
then both groups share the same slope $\beta_1$. This is often called the **parallel slopes assumption**.
...
abline(a = coef(fit3)[1] + coef(fit3)[3], b = coef(fit3)[2],
       lwd = 2, lty = 2)
legend("topleft",
...
In this chapter, we studied one-way ANOVA, two-way ANOVA, and ANCOVA within the general linear model framework. We emphasized that:
