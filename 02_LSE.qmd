# Least Squares Estimation {#sec-lse}
...
After obtaining the LSE, define the **residual
vector**
...
Recall that the LSE satisfies the normal equations $\mathbf{X}^\top\mathbf{X}\hat{\boldsymbol{\beta}}
=
\mathbf{X}^\top\mathbf{Y}.$ If we rearrange, we have
...
## The common name to refer to what we have done -- Ordinary Least Squares (OLS)

Now that we have seen both the algebraic and geometric views of least
squares, we can explain the name **ordinary least squares**, or **OLS**.
...
- **Least:** we choose $\boldsymbol{\beta}$ to make this quantity as
  small as possible.

- **Ordinary:** we use the usual unweighted sum of squared residuals,
  so each observation enters the criterion in the same way.
...
\text{OLS}
=
\text{minimize the ordinary sum of squared residuals}.
}
$$

Geometrically, OLS finds the point in $\mathcal{C}(\mathbf{X})$ that is
closest to $\mathbf{Y}$ using ordinary Euclidean distance.

Later, we will consider extensions such as weighted least squares and
...
What do **ordinary**, **least**, and **squares** each mean in OLS?
...
We defined $\hat{\boldsymbol{\beta}}=(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{Y}$ by solving an optimization problem.

However, under the statistical model, $\mathbf{Y}=\mathbf{X}\boldsymbol{\beta}+\boldsymbol{\varepsilon},$ the response $\mathbf{Y}$ is random. Therefore, $\hat{\boldsymbol{\beta}}$ is also a random vector.

In next chapter, we will study quantities such as

$$
\mathbb{E}
(\hat{\boldsymbol{\beta}}),\quad \text{and}\quad \operatorname{Var}
(\hat{\boldsymbol{\beta}}),
$$
...
In this chapter, we introduced the least squares estimator through the
...
1. Explain why
   ...
   means that the residual vector is orthogonal to the column space
   $\mathcal{C}(\mathbf{X})$.
