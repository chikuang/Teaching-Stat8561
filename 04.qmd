...
::: {.callout-note title="Optional motivation: does measuring tree height add useful information?"}
...
An intercept model with two predictors is fitted to $n=25$ observations using a full-rank design. Write the overall null hypothesis and the F reference degrees of freedom under the normal linear model. If the test rejects, must both individual slope tests also reject?

:::

::: {#sol-overall-f-question}

Here $p=3$, so $H_0:\beta_1=\beta_2=0$ and the reference distribution is $F_{2,22}$. Rejection gives evidence that the two slopes are not jointly zero. It does not show that both are nonzero, and it does not require either individual test to reject at the same level. Joint and individual tests answer different questions, especially when predictors are correlated.
...
Ordinary $R^2$ increases from $1-20/100=0.80$ to $1-19/100=0.81$. Adjusted $R^2$ decreases from $1-(20/8)/(100/9)=0.775$ to $1-(19/7)/(100/9)\approx0.756$. The extra predictor reduces SSE but increases residual mean square, from $2.5$ to about $2.714$. Neither summary alone establishes better prediction on new observations.
