...
Assume the model contains an intercept. The total variation in the response is measured by the **total sum of squares**
...
This is the variation not explained by the fitted regression model. It measures how far the observed responses are from the fitted values.
...
This measures the part of the total variation explained by the regression model. In vector form,
...
To compare sums of squares on a common scale, we divide by their associated degrees of freedom. The **mean square for regression** is
...
From Chapter 3, we know that MSE is an unbiased estimator of $\sigma^2$.
...
Two models are **nested** if the reduced model is obtained by imposing constraints on the full model. For example:
...
The reduced model is nested within the full model because it is obtained by setting $\beta_2 = 0$.
...
Since the full model has more flexibility, it cannot fit worse, so $\mathrm{SSE}_F \le \mathrm{SSE}_R$. The quantity $\mathrm{SSE}_R - \mathrm{SSE}_F$ measures the reduction in error due to adding the extra predictors. This is called the **extra sum of squares** due to the added terms.
...
In multiple regression, sums of squares can be defined in different ways depending on what is being adjusted for. Two common ideas are:
...
This distinction becomes important when predictors are correlated. In this course, the main conceptual priority is to understand the extra sum of squares principle. Details of Type I, Type II, and Type III sums of squares can be introduced later if needed.
...
It measures the proportion of total variation explained by the regression model. For an OLS fit with an intercept and $\mathrm{SST}>0$, its values lie between 0 and 1. If every observed response is identical, this ratio is undefined.
...
Because $R^2$ never decreases when additional predictors are added, it can overstate improvement. A commonly used adjustment is
